...
static inline ulong uvirt_to_bus(ulong adr) {
...
static void * rvmalloc(ulong size)
{
        void * mem;
...
static int lock_user(unsigned long start,unsigned long size,int iov_max,
		     struct iovec *iov)
{
        int iovcnt;
	unsigned long pend, end, pmem, pstart;
	...
	if (!(pmem=uvirt_to_bus(start)))
	        return -EINVAL;
	if (!iov_max) 
	        return -ENOSPC;
	iov->iov_base=(caddr_t) pmem;
	if (pstart==pend)
	{
	        iov->iov_len=size;
		return 1;
	...
	iov->iov_len=PAGE_SIZE-(start&(PAGE_SIZE-1));
	iovcnt++; iov++;
	if (iovcnt==iov_max)
    	        return iovcnt;
	start=pstart+PAGE_SIZE;
...
static void unlock_user(int iovcnt, struct iovec *iov)
{
...
/* read I2C */
static int I2CRead(struct i2c_bus *bus, unchar addr) 
{
	u32 i;
	u32 stat;
	struct bttv *btv = (struct bttv*)bus->data;
...
static int I2CWrite(struct i2c_bus *bus, unchar addr, unchar b1,
		    unchar b2, int both)
{
	u32 i;
	u32 data;
...
/* read EEPROM */
static void readee(struct i2c_bus *bus, unchar *eedata)
{
	int i, k;
...
/* write EEPROM */
static void writeee(struct i2c_bus *bus, unchar *eedata)
{
	int i;
...
static int 
make_vrisctab(struct bttv *btv, dword *ro, dword *re,
	      dword *vbuf, ushort width, ushort height, ushort fmt)
{
        ulong line;
	ulong bpl;  /* bytes per line */
	ulong bl;
	ulong todo;
	dword **rp;
	int inter;
	ulong vadr=(ulong) vbuf;
...
static int 
make_iov_risctab(struct bttv *btv, dword *ro, dword *re,
		 int iov_num, struct iovec *iov, 
		 ushort width, ushort height, ushort fmt)
{
        ulong i;
	ulong bpl;  /* bytes per line */
	dword iovp; 
	...
	*(re++)=BT848_RISC_SYNC|BT848_FIFO_STATUS_FM1; *(re++)=0;
	...
	/* Note that TV line numbering starts with 1 but we start with 0 here.
	   So, for even i we need to output odd lines! */
	for (i=0; i < (height<<(1^inter)); i++)
	{
	        if (inter) 
		        rp= (i&1) ? &re : &ro;
		else 
		...
		if (bpl<=iovl)
		{
			...
			*((*rp)++)=iovp;
			bl=bpl-iovl;
			iov++;
			while (bl>iov->iov_len)
			{
			        *((*rp)++)=BT848_RISC_WRITE|iov->iov_len;
				*((*rp)++)=(dword) iov->iov_base;
				bl-=iov->iov_len;
				iov++;
			}
			*((*rp)++)=BT848_RISC_WRITE|BT848_RISC_EOL|bl;
			*((*rp)++)=(dword) iov->iov_base;
			iovl=iov->iov_len-bl; iovp=bl+(dword)iov->iov_base;   
		}
	}
...
static inline 
void write_risc_segment(dword **rp, ulong line_adr, dword command,
...
static void 
bt848_set_winsize(struct bttv *btv)
{
        ushort format;
	int bpp;
	...
	/* RGB8 seems to be a 9x5x5 GRB color cube starting at
	   color 16
	   Why the h... can't they even mention this in the
	   datasheet???
...
*/
static int 
vgrab(struct bttv *btv, struct gbuf *gb)
{
	dword *ro, *re;
	dword *vbuf=(dword *)(btv->fbuffer+gb->adr);
...
static void 
grab(struct bttv *btv, struct gbuf *gb)
{
        unsigned long mem=gb->adr;
        struct iovec *iovecs;
	int iovcnt;
	dword *ro, *re;
	...
	if (!(iovecs=vmalloc(sizeof(struct iovec)*1024)))
	        return;
	iovcnt=lock_user(mem, 0x144000, 1024, iovecs);
	if (iovcnt<0) 
	{
	        printk("bttv: grab_error: %d\n", iovcnt);
		return;
	}
	/*
	   for (i=0; i<10; i++)
	   printk("bttv: 0x%08x 0x%08x\n", iovecs[i].iov_base, iovecs[i].iov_len);
	   for (i=j-10; i<j; i++)
	   printk("bttv: 0x%08x 0x%08x\n", iovecs[i].iov_base, iovecs[i].iov_len);
	   */

	if (!(ro=(dword *) kmalloc(4072, GFP_KERNEL))) 
	{
	         vfree(iovecs);
		 return;
	}
	if (!(re=(dword *) kmalloc(4072, GFP_KERNEL)))
	{
	        kfree((void *) ro);
		vfree(iovecs);
		return;
	...
	btv->gheight=gb->height;
	btv->gfmt=gb->fmt;
	make_iov_risctab(btv, ro, re, iovcnt, iovecs, 
			 btv->gwidth, btv->gheight, btv->gfmt);
	bt848_set_risc_jmps(btv);;
	btor(3, BT848_CAP_CTL);
	btor(3, BT848_GPIO_DMA_CTL);
	btv->gro=virt_to_bus(ro);
	...
	/* wait for wakeup caused by RISCI */
	interruptible_sleep_on(&btv->capq);
	
	unlock_user(iovcnt, iovecs);
	kfree((void *) ro);
	kfree((void *) re);
	vfree(iovecs);
}
	...
	switch (flags)
	{
		case 0:
			...
			if (users==1)
				find_vga();
			if (!btv->fbuffer)
			  btv->fbuffer=(unchar *) rvmalloc(2*0x144000);
			if (!btv->fbuffer)
                        { 
			  btv->user--;
			  return -EINVAL;
...
*/
static int
bttv_mmap(struct video_device *dev, const char *adr, unsigned long size)
{
	struct bttv *btv=(struct bttv *)dev;
...
static int init_bt848(int i)
{
	...
	if (!btv->vbibuf) 
		return -1;
	if (!(btv->grisc=(dword *) kmalloc(16384, GFP_KERNEL)))
	  return -1;
